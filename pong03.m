...
%called from main loop on every frame
  function movePaddles
    ...
    paddle2(2, 1) = ballY + (PADDLE_H / 2);
    paddle2(2, 2) = ballY + (PADDLE_H / 2);
    paddle2(2, 3) = ballY - (PADDLE_H / 2);
    paddle2(2, 4) = ballY - (PADDLE_H / 2);
    paddle2(2, 5) = ballY + (PADDLE_H / 2);
...
%called from main loop on every frame
  function checkGoal
    ...
    if goal %a goal was made
      %pause(START_DELAY);
      numFrame = 0;
      [paddle1V, ~] = train(PLOT_W, PLOT_H, BALL_RADIUS, ballX, ballY, ballSpeed, ballVector, paddle1, paddle2, winner);
      resetGame;
      if winner > 0 %somebody won
        %pauseGame(['      PLAYER ' num2str(winner) ' IS THE WINNER!!!' 10])
        %disp(['      PLAYER ' num2str(winner) ' IS THE WINNER!!!'] );
        if winner == 1
        ...
        if numGames == maxGames
            quitGame = true;
            disp(['PLAYER 1 WINS FOR ' num2str(player1Wins) ', PLAYER 2 WINS FOR ' num2str(player2Wins)] );
        end
        newGame;
      else %nobody won
      end
...
newGame;
while ~quitGame
  [paddle1V, ~] = train(PLOT_W, PLOT_H, BALL_RADIUS, ballX, ballY, ballSpeed, ballVector, paddle1, paddle2, winner, numFrame);
  moveBall;
  movePaddles;
  %refreshPlot;
  checkGoal;
